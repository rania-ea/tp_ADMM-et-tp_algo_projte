# -*- coding: utf-8 -*-
"""
Created on Tue Oct 21 20:58:39 2025

@author: rania
"""


#algorithm du gradient projete avec calcule gradient a la main pour dim = 2
import numpy as np
# INITIALISATION 
k = 0
epsilon = 10**(-2)
delta = 10**(-11) # si t_l ,t_r sont trop proche
# on pose x**0
x_k = np.array([2,0])

n= np.size(x_k)
def f(x): # fonction cout
    return (x[0] -1 )**2 + (x[1]-2.5)**2
# definition des contrainte
A = np.array([[-1 ,2],[1 ,2],[1 ,-2],[-1 ,0],[0 ,-1]])
b = np.array([2 ,6 ,2 ,0 ,0])

I_2 = [0,1,2,3,4]# on pose I_2 ie les indice des contrainte inactive



for k in range(100):
    # 1 CALCULER I_0(x**k) ie ensemble des contraintes active en x**k
    
    b_1 = A @ x_k
    i = 0
    I_0 = []
    #print(I_0)
    for i in range(np.size(b)):
        if np.abs(b_1[i] - b[i]) <= 10**(-12):
            I_0 = np.append(I_0,i) 
            # ici I_0 est {2,4} 
            #print(I_0)
         
    # 2 FORMER A_0 puis on vérifie si le rang de A_0 est respecter
       
    A_0 = np.empty(np.size(x_k)*np.size(I_0)).reshape(np.size(I_0),np.size(x_k))
    
    
    #print(A_0)
    
    
    for i in range (len(I_0)):
        A_0[i,:] = A[int(I_0[i]),:]
        #print(A_0)

    
    # 3 CALCULE P_0
    normp = 0
    test_break=False
    while normp < epsilon:
        if len(A_0) == 0 :
            P_0 = np.identity(n)
        else:
            P_0 = np.identity(n) - np.transpose(A_0) @ np.linalg.inv( A_0 @ np.transpose(A_0) ) @ A_0
        # print(P_0,'P_k')
        
        
        
        # 4 CALCULE grad_f( x_k) , p**k et d**k
        
        h = 10**(-3) # pas (petit)
        
        grad1 =( f(np.array([x_k[0] + h, x_k[1]])) - f(x_k) )/ h 
        grad2 =( f(np.array([x_k[0] , x_k[1] +h])) - f(x_k) ) / h
        gradf=np.array( [grad1,grad2])
        
        p_k = - P_0 @ gradf
        # print(p_k,'p_k')
        normp = np.linalg.norm(p_k) #norme euclidienne
        
        d_k = p_k / normp
        #print(d_k,'d_k')
        
        # 5 CALCULE DE u_k
        if len(A_0) == 0:
            u_k = np.array([])  # pas de contraintes actives → aucun multiplicateur
            u_k_i = 0
        else:
            u_k = - np.linalg.inv( A_0 @ np.transpose(A_0) ) @ A_0 @ gradf 
            #print(u_k,'u_k')
            u_k_i = np.min(u_k) #on pose u_k_i la plus petite des coordoner de u_k
        
        
        # 6 CONDITION 1 : D'ARRET
        print(normp,"normp")
        if normp < epsilon and u_k_i >= -epsilon :
            
            print( 'STOP')
            test_break = True
            break
        
            
        # 7 CONDITION 2 :
        if normp < epsilon and u_k_i < -epsilon :
            i = np.argmin(u_k_i)
            A_0 = np.delete(A_0,i,0)
            #print('passer par 7')
            #print(A_0)
            
    if test_break == True :
            break    
     # 8 CALCULE T 

     # trouve T l'ensemble des t > 0 tel que x_k + t* d_k est dans X
     
    T=10**6 # on choisie arbritérment un t trés grand pour commencer a trouver le plus petit des t
    for i in I_2:
        term1 = b[i] - A[i,:] @ x_k
        term2 = A[i,:] @ d_k
        if term2 > 10**(-12) and T > term1 / term2:
            T = term1 / term2 
    T = max(0.0, 0.99 * T) # pour éviter de sortir du maxx
    print(T,'T')
     
    # 9 CALCULE gradf2 et t_k AVEC ALGO 1
    i = 0
    t_i = min(T,1)
    #print(t_i)
    t_l = 0
    t_r = T
    beta1 = 10**(-4)
    beta2 = 0.99
    sg = f(x_k + t_i* d_k)
    sd = f(x_k) + t_i * beta1 * np.transpose(gradf) @ d_k
     
    grad10 =( f(np.array([x_k[0]+t_i*d_k[0]+ h, x_k[1]+t_i*d_k[1]])) - f(x_k+t_i*d_k) )/ h 
    grad20 =( f(np.array([x_k[0]+t_i*d_k[0] , x_k[1]+t_i*d_k[1] +h])) - f(x_k+t_i*d_k) ) / h
    gradf1= np.array([grad10,grad20])
    rg = np.transpose(gradf1) @ d_k
    rd = beta2 * np.transpose(gradf) @ d_k
    while sg > sd or rg < rd and i<100:
         if np.abs(t_l - t_r) < delta:
             break
         if sg > sd :
             #print(t_i,t_l,t_r,'prout1')
             t_r = t_i
             t_i = (t_l + t_r) / 2
         if sg <= sd and rg < rd :
             #print(sg,sd,rd,rg,' prout2')
             #print(t_l,t_r)
             t_l = t_i
             t_i = (t_l + t_r) / 2
         i= i+1      ### attention indentation
         sg = f(x_k + t_i* d_k)
         sd = f(x_k) + t_i * beta1 * np.transpose(gradf) @ d_k
         
         grad10 =( f(np.array([x_k[0]+t_i*d_k[0]+ h, x_k[1]+t_i*d_k[1]])) - f(x_k+t_i*d_k) )/ h 
         grad20 =( f(np.array([x_k[0]+t_i*d_k[0] , x_k[1]+t_i*d_k[1] +h])) - f(x_k+t_i*d_k) ) / h
         gradf1= [grad10,grad20]
         rg = np.transpose(gradf1) @ d_k
         rd = beta2 * np.transpose(gradf) @ d_k
    # print(i)
    #10 INCREMENTATION DE x_k
    x_k = x_k + t_i * d_k
    print(x_k,'x_k')
    #print(t_i)
    #print(d_k)
    #print(T)
